...
describe('sheets', () => {
  ...
  beforeEach(() => {
    ...
    allSigninsSpy = jest.spyOn(sheets, 'getAllSheetsSignins').mockImplementation(async () => {
      return sheet.allSignins;
    });
  });
  ...
  describe('getAllSigninsDict()', () => {
    it('should return all signins for the current calendar year', async () => {
      const res = await sheets.getAllSigninsDict(false);
      
      let member = sheet.allMembers[0];
      const test = member[sheets.MEMBER_INDICES.CertificateNumber] + member[sheets.MEMBER_INDICES.Type];

      expect(res[test]).toHaveProperty('id');
      expect(res[test]).toHaveProperty('ts');
      expect(res[test]).toHaveProperty('lastName');
      expect(res[test]).toHaveProperty('droppedOffKids');
      expect(res[test]).toHaveProperty('numberOfGuests');
      expect(res[test]).toHaveProperty('numberOfMembers');
    });
  });
});